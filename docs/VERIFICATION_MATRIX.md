...
| Dynamic and complex actions | Hover/lazy scroll, native controls, context menu, popup, download and opt-in same-origin iframe execute; upload/canvas/rich-text/drag are routed to recording | browser benchmark fixtures and deterministic RD008 boundaries |
...
| Replay and cleanup | Fresh execution distinguishes reproduced, changed, environment-changed, target-missing, and uncertain outcomes; cleanup is idempotent and benchmark-gated | five-outcome browser smoke, benchmark replay, and cleanup smoke |
