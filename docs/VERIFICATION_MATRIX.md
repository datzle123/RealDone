...
| Managed-runtime readiness | CLI/MCP use a dedicated bounded startup-health budget, stop timed-out process trees, return redacted state/PID/restart diagnostics and do not wait after health passes | invalid/bounded/delayed runtime unit controls plus dynamic-port MCP no-URL timeout/cleanup broken case and delayed healthy browser control |
