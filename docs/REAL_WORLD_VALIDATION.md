...
The current checked-in candidate evidence is bound to Phase J fingerprint `0b39d54299d3c50d0f002382e04d569591d56c10c1fca5623d331b374221d047`. Every primary scan was `VALID` and untruncated. Compact documents bind the fresh primary scans, PostgreSQL CRUD scans, cleanup ledger, Level 7 role verification and Codex cycle artifacts by SHA-256. PR run [`30198005122`](https://github.com/datzle123/RealDone/actions/runs/30198005122) passed every Linux/macOS/Windows browser/package job and correctly rejected the earlier Phase I fingerprint at gate 15; the five real cases were then rerun rather than relabelled. The refreshed inputs merged with all six hosted candidate attestations and passed 15/15 locally; Phase J remains a candidate until GitHub repeats and signs that aggregate. Released `v1.3.3` and its independent signed run [`30189340006`](https://github.com/datzle123/RealDone/actions/runs/30189340006) remain the historical published-package qualification.
...
Pocket Ledger was generated in a separate MIT repository by Codex, then exercised through the public scanner and a Level 7 role contract. Member state hid Reset and received API 403; admin state saw Reset. Authenticated Codex Desktop thread `019f8770-219f-7612-81eb-98670009f693` used Codex CLI `0.143.0` to call RealDone MCP for a fresh green baseline, produced run `20260726T105538Z-d5d0` with one selected RD901 regression and an unchanged contract, repaired Increment, and finished in run `20260726T105629Z-4108` with one selected contract and zero regressions. The release validator parses and cross-checks those bound artifacts instead of trusting the agent's final message.
...
Machine-readable candidate inputs are in `release/external-cases.json` and `release/evidence/`; the Phase J aggregate is awaiting the refreshed hosted gate. Hosted run [`30195945498`](https://github.com/datzle123/RealDone/actions/runs/30195945498) remains the signed Phase I history, while run [`30189340006`](https://github.com/datzle123/RealDone/actions/runs/30189340006) independently passed and signed all 15 gates for released `v1.3.3`.
