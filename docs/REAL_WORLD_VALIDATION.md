...
| --- | --- | --- | --- |
| TodoMVC React | `VALID`, 7/7 assets | both standalone Enter mutations executed; React remained `EPHEMERAL` and TypeScript React remained `BROWSER_LOCAL` | 0 |
...
The current checked-in evidence is bound to Phase I candidate fingerprint `f3f65840e53777056fed4fc770853a522da85c46626f5bc910853515c266cc1a`. Every primary scan was `VALID` and untruncated. Compact documents bind the primary scans, PostgreSQL CRUD scans, cleanup ledger, Level 7 role verification and Codex cycle artifacts by SHA-256. Candidate-head run [`30193615824`](https://github.com/datzle123/RealDone/actions/runs/30193615824) already passed its Linux/macOS/Windows, Node 20/22, browser and package jobs; its first aggregate correctly failed because the previous external artifacts belonged to fingerprint `68d4a8cb…`. Combining those same successful platform attestations with the fresh candidate evidence now passes 15/15 locally, but Phase I remains unreleased until hosted CI validates and signs the evidence commit itself. Released `v1.3.3` and its signed run [`30189340006`](https://github.com/datzle123/RealDone/actions/runs/30189340006) remain the historical release qualification.
...
Pocket Ledger was generated in a separate MIT repository by Codex, then exercised through the public scanner and a Level 7 role contract. Member state hid Reset and received API 403; admin state saw Reset. Authenticated Codex Desktop thread `019f8770-219f-7612-81eb-98670009f693` used Codex CLI `0.143.0` to call RealDone MCP for a green baseline, produced run `20260726T081932Z-0ea9` with one selected RD901 regression and an unchanged contract, repaired Increment, and finished in run `20260726T082050Z-ad9f` with one selected contract and zero regressions. The release validator parses and cross-checks those bound artifacts instead of trusting the agent's final message.
...
Machine-readable candidate inputs are in `release/external-cases.json` and `release/evidence/`. For the released package, hosted run [`30189340006`](https://github.com/datzle123/RealDone/actions/runs/30189340006) passed all 15 gates, GitHub signed both aggregate JSON artifacts, and `gh attestation verify` accepted them for `v1.3.3`. Phase I receives its own signed run only after the candidate evidence commit passes hosted aggregation; it does not inherit the prior signature. The earlier `v1.3.2` package path remains qualified by hosted run [`29977292441`](https://github.com/datzle123/RealDone/actions/runs/29977292441).
