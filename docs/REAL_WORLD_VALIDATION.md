...
The current `v1.3.3` candidate engine fingerprint is `68d4a8cb4b087d702df15d609d0ee0f7c92efaf731fdccd0d726b462a9878abe`. Its repository-confined raw scans are stored under `release/evidence/raw/`; every primary case was `VALID` and untruncated. Compact documents are SHA-256-bound to the current engine, primary scans, supporting PostgreSQL CRUD scans, cleanup ledger, role verification and Codex cycle artifacts. The release gate requires semantic observable evidence for all nine §27 capability classes, not merely a minimum case count. Signed hosted run [`30187784286`](https://github.com/datzle123/RealDone/actions/runs/30187784286) passed all 15 gates across Windows/macOS/Linux; npm publication waits for the hosted gates on the versioned release commit.
...
Machine-readable candidate inputs are in `release/external-cases.json` and `release/evidence/`; hosted run [`30187784286`](https://github.com/datzle123/RealDone/actions/runs/30187784286) passed all 15 gates, GitHub signed both aggregate JSON artifacts, and `gh attestation verify` accepted them. For the released `v1.3.2` fingerprint, hosted run [`29977292441`](https://github.com/datzle123/RealDone/actions/runs/29977292441) passed the installed-package path across Windows/macOS/Linux and all 15 gates, and `gh attestation verify` confirmed its source commit `ab902acdfb95463c82a7c842f5b497a68d8c325a`.
