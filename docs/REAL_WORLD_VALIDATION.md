...
The current candidate engine fingerprint is `68d4a8cb4b087d702df15d609d0ee0f7c92efaf731fdccd0d726b462a9878abe`. Its repository-confined raw scans are stored under `release/evidence/raw/`; every primary case was `VALID` and untruncated. Compact documents are SHA-256-bound to the current engine, primary scans, supporting PostgreSQL CRUD scans, cleanup ledger, role verification and Codex cycle artifacts. The release gate requires semantic observable evidence for all nine §27 capability classes, not merely a minimum case count. This candidate is not a release until the hosted signed cross-platform gates pass.
