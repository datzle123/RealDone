...
test("CLI help exposes deep and advanced verification controls", async () => {
  ...
  for (const option of ["--yes", "--full", "--deep", "--trace", "--trace-on-failure", "--video", "--policy", "--storage-state", "--browser-path", "--manage-runtime", "--runtime-startup-timeout", "--environment-timeout", "--allow-iframe", "--sqlite", "--database-config", "--provider-config"]) {
