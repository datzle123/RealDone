...
function findingTimeline(finding: Finding): string {
  ...
  if (evidence.apiReadBack) items.push({ at: evidence.after?.at ?? evidence.durationMs, text: `API read-back ${evidence.apiReadBack.url} → ${evidence.apiReadBack.status ?? "error"}; canary present: ${evidence.apiReadBack.canaryPresent}` });
  if (evidence.executionError) items.push({ at: evidence.durationMs, text: `Execution error: ${evidence.executionError}` });
  if (evidence.traceSuppression) {
    items.push({
...
export function renderHtml(report: ScanReport): string {
  ...
  const environment = report.environment
    ? `<section><h2>Environment health</h2><div class="environment ${verdictClass(report.environment.status === "VALID" ? "VERIFIED" : "BROKEN")}"><strong>${escapeHtml(report.environment.status)}</strong><span>${report.environment.render.interactiveElements} interactive elements · ${report.environment.assets.checked} critical assets checked · ${report.environment.durationMs}ms</span>${report.environment.findings.length > 0 ? `<ul>${report.environment.findings.map((finding) => `<li><code>${escapeHtml(finding.code)}</code><span><b>${escapeHtml(finding.title)}</b><br>${escapeHtml(finding.detail)}${finding.url ? `<br><small>${escapeHtml(finding.url)}</small>` : ""}</span></li>`).join("")}</ul>` : ""}</div></section>`
    : "";
  const selection = report.completeness?.selection;
  const coverage = selection
    ? `<section><h2>Action coverage</h2><div class="coverage ${report.completeness?.truncated ? "partial" : "complete"}">
        <strong>${selection.selectedActions} of ${selection.eligibleActions} eligible actions selected</strong>
        <span>${selection.selectedPages}/${selection.eligiblePages} routes represented · ${selection.omittedActions} omitted by the action budget</span>
        <span>Selected kinds: ${escapeHtml(selection.selectedKinds.join(", ") || "none")} · intents: ${escapeHtml(selection.selectedIntents.join(", ") || "none")}</span>
        <small>${report.completeness?.truncated ? `Partial scan (${escapeHtml(report.completeness.reasons.join(", "))}). Increase the budget or use --full for broader coverage.` : `Complete within the configured budget using ${escapeHtml(selection.strategy)}.`}</small>
      </div></section>`
    : "";
  const selectedMeta = selection
    ? `<span>${selection.selectedActions}/${selection.eligibleActions} eligible actions selected</span>`
    : "";
  return `<!doctype html>
<html lang="en">
<head>
