...
function injectionSource(startedAt: number): string {
  return `(() => {
    ...
    document.addEventListener('change',event=>{const element=event.target;if(!(element instanceof HTMLInputElement||element instanceof HTMLTextAreaElement||element instanceof HTMLSelectElement)||!visible(element)||element.type==='file')return;const hint=((element.name||'')+' '+nameFor(element)).trim();const sensitive=element.type==='password'||/password|passwd|secret|token|api.?key/i.test(hint);const secretEnv=sensitive?'REALDONE_'+(hint||'SECRET').toUpperCase().replace(/[^A-Z0-9]+/g,'_').replace(/^_|_$/g,''):undefined;const type=element instanceof HTMLSelectElement?'select':(['checkbox','radio'].includes(element.type)?'check':'fill');emit({type,pageUrl:location.href,atMs:Date.now()-startedAt,fingerprint:fingerprint(element),...(type==='check'?{checked:element.checked}:{value:sensitive?'[REDACTED]':element.value}),...(secretEnv?{secretEnv}:{})});},true);
  })();`;
}
