...
export function renderContractVerification(result: ContractVerification): string {
  ...
  const performance = result.performance
    ? `<section class="performance"><strong>Performance ${result.performance.passed ? "passed" : "failed"}</strong><p>${result.performance.verificationMs}ms total · ${result.performance.maxStepMs}ms slowest step · ${result.performance.memoryDeltaMb}MB memory delta</p>${result.performance.violations.map((violation) => `<p class="bad">${escapeHtml(violation)}</p>`).join("")}</section>`
    : "";
  const artifacts = result.artifacts
    ? `<section class="performance"><strong>Full evidence</strong><p>${[
        ...result.artifacts.traces.map((file) => `<a href="${escapeHtml(file)}">Trace</a>`),
        ...result.artifacts.videos.map((file) => `<a href="${escapeHtml(file)}">Video</a>`),
        ...(result.artifacts.traceSuppressions ?? []).map((suppression) => `Trace suppressed by secret-safety inspection (${escapeHtml(suppression.reason)}; ${escapeHtml(suppression.findingKinds.join(", "))})`),
        ...(result.artifacts.visualSuppressions ?? []).map((suppression) => `${suppression.artifact === "screenshot" ? "Screenshot" : "Video"} suppressed by visual-privacy policy (${escapeHtml(suppression.reason)})`),
      ].join(" · ")}</p></section>`
    : "";
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>RealDone verify · ${escapeHtml(result.contractName)}</title><style>body{margin:0;background:#0a0e12;color:#eef3f6;font:15px/1.5 system-ui}.wrap{width:min(980px,calc(100% - 32px));margin:0 auto;padding:52px 0}header{margin-bottom:34px}.eyebrow{color:#39d98a;font:700 12px ui-monospace,monospace;letter-spacing:.14em}h1{font-size:42px;letter-spacing:-.04em;margin:14px 0}.summary{color:#96a4b2}.badge{display:inline-block;padding:7px 11px;border-radius:99px;font:700 12px ui-monospace,monospace;background:${result.passed ? "#123424;color:#67e6a8" : "#3a1720;color:#ff8798"}}.performance{border:1px solid #27313c;border-radius:12px;padding:14px;margin:18px 0;background:#11171e}.performance a{color:#7bc5ff}.step{border:1px solid #27313c;border-radius:12px;background:#11171e;margin:10px 0;overflow:hidden}.step summary{display:grid;grid-template-columns:96px 1fr auto;gap:12px;padding:15px;cursor:pointer}.step summary span{font:700 11px ui-monospace,monospace}.step.passed summary span,.ok{color:#39d98a}.step.failed summary span,.bad{color:#ff647c}.step.skipped summary span{color:#ffbf69}.step>div{border-top:1px solid #27313c;padding:4px 15px 15px}.step time{color:#96a4b2}ul{list-style:none;padding:0}li{padding:4px 0}li small{display:inline-block;margin-right:7px;color:#7bc5ff;font:700 10px ui-monospace,monospace;letter-spacing:.08em}code{color:#7bc5ff}</style></head><body><main class="wrap"><header><div class="eyebrow">REALDONE / RECORDED VERIFICATION</div><h1>${escapeHtml(result.contractName)}</h1><span class="badge">${result.passed ? "PASSED" : "FAILED"}</span><p class="summary">${escapeHtml(result.verificationId)} · ${escapeHtml(result.browser)} · ${result.deep ? "deep fresh-context" : "standard"} · roles ${escapeHtml(result.roles.join(", "))} · ${result.steps.length} steps</p></header>${performance}${artifacts}${rows}</main></body></html>`;
}
